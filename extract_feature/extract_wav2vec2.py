...
def handle_iemocap(model: Wav2vec2):
    matroot = '/hy-tmp/DAIC_utterances'
    save_L12 = '/hy-tmp/DAIC_feature/w2v2_large_L12_mat'
    save_L24 = '/hy-tmp/DAIC_feature/w2v2_large_L24_mat'
...
def handle_meld(model: Wav2vec2):
    ...
    for s in state:
        ...
        print(f'We have {len(mats)} samples in total.')
        for mat in mats:
            wavfile = f'/148Dataset/data-chen.weidong/meld/audio/{s}/{mat}'
...
def handle_daic(model: Wav2vec2):
    ...
    # mats = os.listdir(matroot)
    mats = pd.read_csv("~/SpeechFormer-master/metadata/metadata_daicwoz_crop_resample.csv")["name"].to_numpy()
    
    print(f'We have {len(mats)} samples in total.')
    for mat in mats:
        wavfile = f'/hy-tmp/DAIC_utterances/{mat}'
        savefile_L12 = os.path.join(save_L12, mat[:-10] + '.mat')
        savefile_L24 = os.path.join(save_L24, mat[:-10] + '.mat')
        extract_w2v2(model, 12, wavfile, savefile_L12)
        extract_w2v2(model, 24, wavfile, savefile_L24)
...
if __name__ == '__main__':
    ...
    ckpt_path = "./pre_trained_model/wav2vec2/libri960_big.pt"
