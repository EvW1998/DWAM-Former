...
def modify_config(cfg, args):
    ...
    modeltype = cfg.model.type  # DWAM-Former
    database = cfg.dataset.database  # daic_woz
    feature = cfg.dataset.feature  # hubert

    train_config = json.load(open(f'./config/train_{modeltype}.json', 'r'))[database][feature]
    # {'train.EPOCH': 70, 'train.batch_size': 16, 'train.lr': 0.00075}
    model_config = json.load(open('./config/model_config.json', 'r'))[modeltype]
    # {'num_layers': [2, 2, 4, 4], 'expand': [1, 1, 2, -1], 'num_heads': 8, 'dropout': 0.1, 'attention_dropout': 0.1}
