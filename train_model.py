...
class Engine():
    def __init__(self, cfg, local_rank: int, world_size: int):
        ...
        with open('./config/model_config.json', 'r') as f1, open(
                f'./config/{self.cfg.dataset.database}_feature_config.json', 'r') as f2:
        ...
        self.optimizer = torch.optim.SGD(filter(lambda x: x.requires_grad, self.model.parameters()),
                                         lr=self.cfg.train.lr, momentum=0.9)
        self.loss_func = torch.nn.CrossEntropyLoss()  # TODO: change loss
    ...
    def train_epoch(self):
        ...
        for data in pbar_train:
            ...
            phonemes_info = data[-2]
            masks = data[-1]

            self.optimizer.zero_grad()

            out = self.model(x, phonemes_info, masks)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.local_rank == 0:
                ...
                # print(self.test_score_3)
                if len(self.test_score_3) != 0:
                    if score_3 > max(self.test_score_3):
                        self.model_save(is_best=True)
                        print(score_3, score_3)
                    else:
                        print(max(self.test_score_3), score_3)
                else:
                    self.model_save(is_best=True)
                    print(score_3, score_3)
...
def main_worker(local_rank, cfg, world_size, dist_url):
    ...
    if cfg.dataset.database == 'daic_woz':
        cfg.train.vote = True
        folds = [1]
    else:
