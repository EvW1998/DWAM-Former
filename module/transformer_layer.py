    ...
    def forward(self, query, key=None, value=None, key_padding_mask=None, attn_mask=None):
        ...
        if key_padding_mask is not None:
            key_padding_mask = key_padding_mask.unsqueeze(
                1) if key_padding_mask.dim() == 3 else key_padding_mask.unsqueeze(1).unsqueeze(2)
...
class TransformerEncoder(nn.Module):
    def __init__(self, self_attn, embed_dim, qdim=None, kdim=None, ffn_embed_dim=2304, num_heads=8, dropout=0.1,
    ...
    def forward(self, query, key=None, value=None, query_position=None, key_position=None, key_padding_mask=None,
                attn_mask=None):
        """
...
class Transformer(nn.Module):
    ...
    def __init__(self, self_attn, num_layers, embed_dim, qdim=None, kdim=None, ffn_embed_dim=2304, num_heads=8,
                 dropout=0.1, attention_dropout=0.1, activation='relu', use_position=False):
        super().__init__()
        self.self_attn = self_attn
        ...
        self.layers = nn.ModuleList([TransformerEncoder(self_attn, embed_dim, qdim, kdim, ffn_embed_dim, num_heads,
                                                        dropout, attention_dropout, activation) for _ in
                                     range(num_layers)])
...
def build_transformer(self_attn, num_layers, embed_dim, qdim=None, kdim=None, ffn_embed_dim=2304, num_heads=8,
                      dropout=0.1, attention_dropout=0.1, activation='relu', use_position=False):
    '''
    ...
    return Transformer(self_attn, num_layers, embed_dim, qdim, kdim, ffn_embed_dim, num_heads, dropout,
                       attention_dropout, activation, use_position)
